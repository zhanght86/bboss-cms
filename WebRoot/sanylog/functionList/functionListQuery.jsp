...
<script type="text/javascript">
	...
	//查询浏览统计列表数据(页面加载时会加载这个方法，页面调用查询时也会调用)
	 function queryList() {	
		...
		var functionName = $("#functionName").val();
	   	...
	   	$("#custombackContainer").load("showFunctionList.page #customContent", { appId:appId, functionCode:functionCode,functionName:functionName}, function(){loadjs()});
	}
	//查询相应的模块
	function queryAppInfo() {
		$.ajax({
	 	 	type: "POST",
			url : "<%=request.getContextPath()%>/sanylog/functionList/getAllApp.page",
			data :{},
			dataType : 'json',
			async:false,
	...
	function modifyOrIncrementRecord(id,type){
		var url='<%=request.getContextPath()%>/sanylog/functionList/modifyOrIncrementRecord.page?id='+id+'&type='+type;
		if("modify"==type){
			var title='功能清单修改';
		}else{
			var title='功能清单新增';
	...
	function batchInput(){
		var url='<%=request.getContextPath()%>/sanylog/functionList/functionListbatchInput.jsp';
		var title='功能清单导入';
		$.dialog({ id:'iframeNewId', title:title,width:450,height:330, content:'url:'+url}); 
	 ...
	 function staticSpentTime(){
		 $.ajax({
		 	 	type: "POST",
				url : "<%=request.getContextPath()%>/sanylog/functionList/staticSpentTime.page",
				data :{},
				dataType : 'json',
				async:false,
				beforeSend: function(XMLHttpRequest){
	...
	<sany:menupath />
		<div id="searchblock">
			...
			<div class="title_box">
				...
				<a href="#" class="bt_small" id="SubjectInputButton" onclick="batchInput()"><span>功能清单导入</span></a>
				</div>
			</div>
		</div>
		<div class="title_box">
			<strong>功能清单列表</strong>
		</div>
