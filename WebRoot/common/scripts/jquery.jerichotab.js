...
$.extend($.fn, {
    initJerichoTab: function(setting) {
        var opts = $.fn.extend({
            ...
            uniqueId: null,
            //the max length of tabs that can been opened once
            maxtabs: 100,
        ...
        function createJerichoTab() {
            ...
            */
            //var jerichotab = $('<div class="jericho_tab" style="border: 2px solid #ff0000;"><div class="tab_pages submenuj" style="border: 2px solid #00ff00;"><div class="tabs" style="border: 2px solid #0000ff;"><ul /></div></div><div class="tab_content" style="border: 2px solid #ffee00;"><div id="jerichotab_contentholder" class="content" style="border: 2px solid #ff00aa;"/></div></div>')
             //                               .appendTo($(opts.renderTo));
            var jerichotab = $('<div class="jericho_tab"><div class="tab_pages submenuj"><div class="tabs"><ul /></div></div><div class="tab_content"><div id="jerichotab_contentholder" class="content" /></div></div>')
            .appendTo($(opts.renderTo));
            ...
            $.fn.jerichoTab = {
                ...
                tabPageWidth: $('.tab_pages', jerichotab).width() - 25,
                loader: opts.loader,
                ...
                addTab: function(tabsetting) {
                    ...
                    if (ps.tabFirer != null) {
                        ...
                        if (typeof $('#jerichotab_' + curId) != 'undefined' && $('#jerichotab_' + curId).length > 0) {
                        	$('#jerichotab_' + curId).attr('dataLink',ps.data.dataLink);
                            return $.fn.setTabActive(curId).adaptSlider().loadData();  
                        }
                        else if (tabHash.length == opts.maxtabs){
                        	//alert("打开页面数已超过最大限制,请先关闭部分页面后再操作！");
                        	//返回当前被选中的TAB
                        	return  $.fn.jerichoTab.tabpage.children('li').filter('.tab_selected').adaptSlider();
                        } else {
	                        //ps.tabFirer.attr('jerichotabindex', curIndex);
	                        ps.tabFirer.attr('jerichotabindex', curId);
                        }
                    }
                    //newTab html tree:
                    /*
                    ...
                    */
                    ...
                    var newTab = $('<li class="jericho_tabs tab_selected" style="width:0px"  id="jerichotab_' + curId + '" dataType="' + ps.data.dataType + '" dataLink="' + ps.data.dataLink + '">' +
                                                    '<div class="tab_left">' +
                                                        '<div class="tab_icon" style="' + (ps.iconImg == '' ? 'backrground:none' : 'background-image:url(' + ps.iconImg + ')') + '">&nbsp;</div>' +
                                                        '<div class="tab_text" title="' + ps.title + '"  style="width:' + (opts.tabWidth - 30) + 'px"  >' + ps.title.cut(opts.tabWidth / 10 - 1) + '</div>  ' +
                                                            ...
                                                            .applyHover()
                                                                .applyCloseEvent()
                                                                        .animate({ 'opacity': '1', width: opts.tabWidth }, function() {
                                                                            $.fn.setTabActive(curId);
                                                                        }).dblclick(function(){
                                                                        	$('.tab_close>a', this).mouseup();
                                                                        });
                    ...
                    tabHash.push({
                        ...
                        tabId: 'jerichotab_' + curId,
                        holderId: 'jerichotabholder_' + curId,
                        iframeId: 'jerichotabiframe_' + curId,
                        onCompleted: ps.onLoadCompleted
                    });
                    
                    if($('li[id^="jerichotab_"][id$="Node"]').length == 1) {
                    	$.fn.jerichoTab.tabpage.parent().before($('<span id="divClose" class="jericho_close_but" title="全部关闭"/>').click(function() {
                    		$('li[id^="jerichotab_"][id$="Node"]').each(function(i){
            ...
            $.each(opts.tabs, function(i, n) {
                $.fn.jerichoTab.addTab(n).loadData();
            });
            if (tabHash.length == 0)
...
//          $.fn.jerichoTab.tabPageWidth = $(".tab_pages", $.fn.jerichoTab.master).width() - (($(".jericho_slider").length > 0) ? 38 : 0);
        	$.fn.jerichoTab.tabPageWidth = $(window).width() - 40 - (($(".jericho_slider").length > 0) ? 38 : 0);
    ...
    buildIFrame: function(src) {
        return this.each(function() {
            ...
            var iframe = $('<iframe id="' + jerichotabiframe + '" name="' + jerichotabiframe + '" src="' + src + '" frameborder="0" scrolling="auto" />')
                                        .css({ width: '100%', height: $(divTarget).height() -23, border: 0 })
                                            .appendTo($(this));
    ...
    applySlider: function() {
        return this.each(function() {
            ...
            var selectTab = $.fn.jerichoTab.tabpage.children('li').filter('.tab_selected');           
            // get index of the selected tab 
            var selectIndex = $("#"+selectTab.attr("id")).index();            
            var leftMsWidth = selectIndex * $.fn.jerichoTab.tabWidth;
            if (tabHash.length > 0 && offWidth > 0) {
                ...
                $.fn.jerichoTab.tabpage.css({ width: offWidth + $.fn.jerichoTab.tabPageWidth - 38 }).animate({ left: -(leftMsWidth<offWidth?leftMsWidth:offWidth) }, function() {
                	//append 'jerichosliders' to the tabpageholder if 'jerichoslider' has't been added
                    if (tabHash.length > 1 && $('.jericho_sliders').length <= 0) {
                    	...
                    	selectTab = $.fn.jerichoTab.tabpage.children('li').filter('.tab_selected');    
                    	// the selected tab is last tab
                    	if (selectTab.attr("id") ==tabHash[tabHash.length-1].tabId){
                    		setSlider('right', false);
                    		setSlider('left', true);
                    	} else if (selectTab.attr("id") ==tabHash[0].tabId) {
                    		// the selected tab is first tab
                    		setSlider('right', true);
                    		setSlider('left', false);
                    	} else {
                    		// the selected tab is not first or last tab
                    		setSlider('left', true);
                    		setSlider('right', leftMsWidth<offWidth? true:false);
                    	}                 	
                    }
            ...
            switch ($.fn.jerichoTab.loader) {
                ...
                case 'righttag':
                    ...
                    if ($('#jerichotab_contentholder>.righttag').length == 0)
                        $('<div class="righttag">加载中...</div>').appendTo($(this));
                    else
