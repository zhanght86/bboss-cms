...
function PopulateTB(y) {
	...
	for (i=0; i<elements.length; i++) {
		...
		default:
			//modify by xinwang.jiao 2007-9-18
			//??????+ ??-??????????????????????????????????
...
function onKeyDown(event){
	//if(eWebEditor.document.value.length>=10) {
		//alert("需要换行了！");
		//}
...
function format(what,opt) {
	//????????????
	if(what.toLowerCase() != "selectall" && !validateMode()){return;}
	eWebEditor.focus();
	if (!history.saved){saveHistory();}
	if(opt=="RemoveFormat"){
		what=opt;
...
//	}
	text = text.replace(/<CMS_PAGE_SEPARATOR><\/CMS_PAGE_SEPARATOR>/g, "<img src=\"buttonimage/blue/page.gif\" id=\"_CMSPage\">");
	return text;
...
function insertTRSPage(){
	...
	//focus
	eWebEditor.focus();
	insertHTML("<img src=\"buttonimage/blue/page.gif\" id=\"_CMSPage\">");
}
...
// ???????????????? ??????????????????????????????????
// add by xinwang.jiao 2007-9-13
function ShowDialogCMS(url, width, height, optValidate) {
	if (optValidate) {
		if (!validateMode()) return;
	}
	eWebEditor.focus();
	url = "../eWebeditor/dialog/img_cms.htm"
...
function Maximize() {
	if (!validateMode()) {return;}
	saveHistory();
	window.open("dialog/fullscreen.htm?style="+config.StyleName+"&cusdir="+config.CusDir, 'FullScreen'+sLinkFieldName, 'toolbar=no,location=no,directories=no,status=yes,menubar=no,scrollbars=yes,resizable=yes,fullscreen=yes');
}
...
function insert(what) {
	...
	case "br":
		...
		break;
	case "code":
		insertHTML('<table width=95% border="0" align="Center" cellpadding="6" cellspacing="0" style="border: 1px Dotted #CCCCCC; TABLE-LAYOUT: fixed"><tr><td bgcolor=#FDFDDF style="WORD-WRAP: break-word"><font style="color: #990000;font-weight:bold">????????????????</font><br>'+HTMLEncode(sel.text)+'</td></tr></table>');
		break;
	case "quote":
		insertHTML('<table width=95% border="0" align="Center" cellpadding="6" cellspacing="0" style="border: 1px Dotted #CCCCCC; TABLE-LAYOUT: fixed"><tr><td bgcolor=#F3F3F3 style="WORD-WRAP: break-word"><font style="color: #990000;font-weight:bold">????????????????</font><br>'+HTMLEncode(sel.text)+'</td></tr></table>');
		break;
	case "big":
