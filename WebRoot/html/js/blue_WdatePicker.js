...
if (scriptTags != null && scriptTags.length != 0) {
	for (var i=0; i<scriptTags.length; i++) {
		if (scriptTags[i].src != "" && scriptTags[i].src.indexOf('blue_WdatePicker') != -1 && scriptTags[i].src.indexOf('lang') != -1) {
			language = scriptTags[i].src.substr(scriptTags[i].src.lastIndexOf('lang')+5);
			break;
		}
	}
}
