...
import com.frameworkset.platform.sysmgrcore.web.javabean.tree.TreeControl;
import com.frameworkset.platform.sysmgrcore.web.javabean.tree.TreeControlNode;
 ...
 * should be sent, with a string "<code>{name}</code> marking where the node
 ...
 * @version $Revision: 303561 $ $Date: 2004-12-01 22:49:01 -0500 (Wed, 01 Dec
 *          2004) $
 */

public class TreeControlTag extends TagSupport implements Serializable {
     ...
     * tree nodes. The placeholder "<code>{name}</code>" will be replaced by
     * the <code>name</code> property of the current tree node.
     */
    ...
    protected void render(JspWriter out, TreeControlNode node, int level,
        ...
        String updateTreeAction = replace(getAction(), "tree={name}", "select="
                + encodedNodeName);
