...
public class ResourceTypeCMSTree extends COMTree implements Serializable
{

    public ResourceTypeCMSTree()
    {
    }
    ...
    public boolean hasSon(ITreeNode father)
    {
        ...
        if(father.getType().equals("resourceType") || father.getType().equals("resourceType_auto"))
        {
            ...
            if (!hasson) { //如果没有资源类型判断资源的数据结构 如果是列表资源直接返回false
                if (resourceManager.getResourceInfoByType(id).getStruction().
                    ...
                    String sql  = "select count(*) from td_sm_res where restype_id='" + id + "' and parent_id='-1'";
        ...
        {
            String sql  = "select count(*) from td_sm_res where parent_id='" + id + "'";
                    DBUtil dbUtil = new DBUtil();
                    try {
                        dbUtil.executeSelect(sql);
                        if(dbUtil.getInt(0,0) > 0)
    ...
    public boolean setSon(ITreeNode father, int curLevel)
    {
        ...
        String path = "";
        String curSystem = super.accessControl.getCurrentSystemID();
        ...
        if(father.isRoot() || father.getType().equals("resourceType") || father.getType().equals("resourceType_auto"))
        {
            ...
            ResourceInfo resourceInfo = resourceManager.getResourceInfoByType(id);
//            System.out.println("resourceInfo:" + resourceInfo.isAuto());
...
//                System.out.println("res.isAuto() = " + res.isAuto());
                //判断当前系统模块；
                if (res.isUsed() && res.containSystem(curSystem) && !res.isAuto() && res.maintaindata()) {
                    treeid = res.getId();
                    treeName = res.getName();
                    /*
                    if(treeName.equals("菜单资源") || treeName.equals("tab资源") 
                    		|| treeName.equals("频道资源") || treeName.equals("机构资源")
                    		|| treeName.equals("用户资源") || treeName.equals("资源类资源")
                    		|| treeName.equals("用户组资源") || treeName.equals("角色资源")
                    		|| treeName.equals("字典资源") || treeName.equals("岗位资源"))
                    	continue;
                    	*/
                    ...
                    showHref = true;
                    if (!res.isAuto())
//                        addNode(father, treeid, treeName, resType + "_auto",
...
//                    else
             ...
             * 如果是树型的数据源则设置树型数据资源
             */
            if(resourceInfo != null && resourceInfo.getStruction().equals("tree"))
            {
                DBUtil dbUtil = new DBUtil();
                try {
                    dbUtil.executeSelect(
                        "select * from td_sm_res where restype_id='" + id + "' and parent_id='-1'");
                    ...
                    for(int i = 0; i < dbUtil.size(); i ++)
                    {
                        ...
                        params.put("nodeLink", "/sysmanager/resmanager/resource_toolbar.jsp");
        ...
        else
        {
            ...
            try {
                dbUtil.executeSelect(
                    "select * from td_sm_res where parent_id='" + id + "'");
                resType = "application_resource";
                ...
                for(int i = 0; i < dbUtil.size(); i ++)
                {
                    Map params = new HashMap();
                    //输出资源类型名称
                    ResourceInfo resourceInfo = resourceManager.getResourceInfoByType(dbUtil.getString(i, "restype_id"));
