	    ...
	    if (checks != ""){
	    	if (dealType==1){
	    		...
	    		W.$.dialog.confirm(outMsg, function() {
        			...
        			userSelf+"&userRole="+userRole+"&flag=1&userJob="+userJob+"&curOrgId=<%=curOrgId%>";
					orgForm.submit();
	 				return true;
	    		})
			} 
...
<div align="center">
	...
	<tr><td height='40' class="detailtitle" align=center colspan=3><b><pg:message code="sany.pdp.role.copy"/></b></td></tr>
		<pg:listdata dataInfo="com.frameworkset.platform.sysmgrcore.web.tag.UserResCopyList" keyName="UserResCopyList"/>
		<!--分页显示开始,分页标签初始化-->
				...
				<pg:notequal actual="${UserResCopyList.itemCount}"  value="0">
			      ...
			      %>	
			      		<tr class="labeltable_middle_tr_01" onMouseOver="this.className='mousestyle1'" onMouseOut="this.className= 'mousestyle2'">	      				
			      		    ...
			      		    <td  align=left><input onClick="checkOne('checkBoxAll','ID')" type="checkbox" name="ID<%if(state){%>disabled<%}%>" value="<pg:cell colName="userId" defaultValue=""/>" disabled="true"></td>					
							<td  align=left>
