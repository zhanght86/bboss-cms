...
function dhtmlXComboFromSelect(parent,size){
   ...
   for (var i=0; i<parent.options.length; i++){
      ...
      var szm=parent.options[i].getAttribute("pyvalue");
      if ((typeof(val)=="undefined")||(val===null)) val=label;
      x[i]={value:val,text:label,img_src:parent.options[i].getAttribute("img_src"),pyvalue:szm};
   }
...
*/
   dhtmlXCombo.prototype.disable = function(mode){
      ...
      this._disabled=z;
      if(mode){
      	document.getElementById("img"+this.name+"s").style.filter="gray";
      }else{
      	document.getElementById("img"+this.name+"s").style.filter="";
      }
   }
/**
...
*/
      ...
      dhtmlXCombo.prototype._onKey = function(e){
         ...
         if(ev==8&&isro==true){
         	if(jianzhi.length>1){
         		jianzhi=jianzhi.substring(0,jianzhi.length-1);
         	}else{
         		jianzhi='';
         	}
         ...
         if (ev=="40"){  //down
            ...
            this.selectNext(-1);
         	...
         	if(ev==8){
         		...
         		var tozhi=String.fromCharCode(ev);
         		if(ev==189){
         		tozhi=(isshift==true)?"_":"-";
         		}else if(parseInt(ev)>95&&parseInt(ev)<106){
         		tozhi=parseInt(ev)-96;
         		}
         		jianzhi+=tozhi;
         	}
...
*/
    ...
    dhtmlXCombo.prototype.filterSelf = function(mode)
   {
      ...
      var text='';
	  if(this.DOMelem_input.readOnly){
	   isro=true;
	   text=jianzhi;
	  }else{
	   isro=false;
      ...
      for(var i=0; i<this.optionsArr.length; i++){
      	if(isro){
      	var z=filter.test(this.optionsArr[i].pyvalue);
      	}else{
...
{
   var z=document.createElement("<img id='img"+self.name+"' />");
