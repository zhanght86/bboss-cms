...
var superTable = function(tableId, options) {
	...
	if (this.sDataTable.getElementsByTagName("COLGROUP").length > 0) {
		this.sDataTable.removeChild(this.sDataTable
	...
	for (i = 0, j = alpha.length; i < j; i++) {
		...
		for (k = 0, m = alpha[i].cells.length; k < m; k++) {
			if (alpha[i].cells[k].colSpan !== 1
					|| alpha[i].cells[k].rowSpan !== 1) {
	...
	this.sDataTable.insertBefore(this.sColGroup.cloneNode(true),
			this.sDataTable.firstChild);
	this.sHeaderTable.insertBefore(this.sColGroup.cloneNode(true),
			this.sHeaderTable.firstChild);
	if (this.fixedCols > 0) {
		this.sFDataTable.insertBefore(this.sColGroup.cloneNode(true),
				this.sFDataTable.firstChild);
		this.sFHeaderTable.insertBefore(this.sColGroup.cloneNode(true),
				this.sFHeaderTable.firstChild)
	}
	if (this.cssSkin !== "") {
		...
		if (this.fixedCols > 0) {
			...
			this.sFHeaderTable.className += " " + this.cssSkin
					+ "-FixedHeaders"
		}
	}
	if (this.fixedCols > 0) {
	...
	this.sHeaderHeight = this.sDataTable.tBodies[0].rows[(this.sDataTable.tHead) ? 0
			: this.headerRows].offsetTop;
	sDataTableStyles = "margin-top: " + (this.sHeaderHeight * -1) + "px;";
	sDataStyles = "margin-top: " + this.sHeaderHeight + "px;";
	sDataStyles += "height: " + (this.sParentHeight - this.sHeaderHeight)
			+ "px;";
	if (this.fixedCols > 0) {
		...
		if (window.getComputedStyle) {
			...
			if (navigator.taintEnabled) {
				this.sFHeaderWidth += Math.ceil(parseInt(alpha
						.getComputedStyle(beta, null).getPropertyValue(
								"border-right-width")) / 2)
			} else {
				this.sFHeaderWidth += parseInt(alpha.getComputedStyle(beta,
...
function initTableFn(opt) {
	superTable_demo = new superTable(opt.table, {
		cssSkin : "sDefault",
		fixedCols : opt.fixedCols
	});
	...
	if (width <= 1024) {
		var container = document.getElementById(opt.div);
		superTable_demo.sData.style.height = "232px";
	}
	if (width > 1024 && width <= 1440) {
		var a = document.getElementById(opt.div);
		a.style.height = '362px';
		superTable_demo.sData.style.height = "337px";
	}
	if (width > 1440) {
		var container2 = document.getElementById(opt.div);
		container2.style.height = "600px";
		superTable_demo.sData.style.height = "475px";
	}
}

function obj(menu) {
	return (navigator.appName == "Microsoft Internet Explorer") ? this[menu]
...
function togglevisible(treepart) {
	if (this.obj("T" + treepart).style.visibility == "hidden") {
		...
		var width = screen.width;
		superTable_demo.sData.style.width = (superTable_demo_sData_width)
				+ "px";
		superTable_demo.sFHeader.style.width = (superTable_demo_sHeader_width)
				+ "px";
	} else {
		var change = document.getElementById("change");
		change.style.marginLeft = '15px';
		superTable_demo.sData.style.width = (superTable_demo_sData_width + 195)
				+ "px";
		superTable_demo.sFHeader.style.width = (superTable_demo_sHeader_width + 195)
				+ "px";
