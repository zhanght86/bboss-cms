(function (){
	var f=document,g=window,l=navigator,o=location,m=screen,h=encodeURIComponent,i=decodeURIComponent,j="https:"==document.location.protocol?"https:":"http:",n=function (){
		...
		this.pic="";
		this.lpic="none";
		this.NR=j+"//c.cnzz.com/cnzz_core.php";
		this.online="";
		this.cookiemax=2048;
		this.error_log="_CNZZ_error_log";
		this.server_now="1351562494";
		this.move_server="g23.cnzz.com";		
		this.init();
	...
	n.prototype={
		init:function (){
			...
			this.lg=l.systemLanguage||l.language;
			this.lg=this.lg.toLowerCase()
		},getRepeatIP:function (){
			var a=new Date,c=new Date,b=this.getSubCookiePart("cnzz_a");
			if(null===b)b=0;
			else {
				var d=this.getSubCookiePart("retime")||
				this.now,d=parseInt(d);
				a.setTime(d);
				c.setTime(this.now);
			...
			i(this.refer).split("/")[2]!=f.domain&&(this.sin=this.refer)
		},getRes:function (){
			this.res=
			0
		},getCNZZeid:function (){
			...
			if(-1<c){
				b=f.cookie.indexOf(";",c);
				if(-1==b)b=f.cookie.length;
				b=f.cookie.substring(c+a.length,b);
				if(0<b.length){
					a=b.split("&");					
					c=0;
			...
			var a=(o.hostname+"/").match(/[\w-]+\.(com|net|org|gov|edu|mil|cc|biz|name|info|mobi|cn|int|pro|museum|coop|aero|xxx|idv)(\.(cn|hk|jp|tw|kr|mo))*\//ig);
			if(a){
		...
		c,b){
			...
			var c=this.subCookieParts[0],b=this.subCookieParts[4],a=a-(this.cookiemax-4),d=c.length-512;
			0<d?(this.subCookieParts[0]=c.substr(0,512),a>d&&(this.subCookieParts[4]=b.substr(0,this.sin.length-(a-d)))):this.subCookieParts[4]=b.substr(0,b.length-a)
			...
			for(var c=a.length,b=0;b<c;b++)a[b]&&f.write(unescape(a[b]))
		},sendData:function (){
			this.move_server&&this.callRequest([j+"//g23.cnzz.com/stat.htm?id=3269520&r="+this.refer+"&lg="+this.lg+"&ntime="+this.ntime+"&repeatip="+this.repeatip+"&rtime="+this.rt+"&cnzz_eid="+this.eid+"&showp="+this.showp+"&st="+this.st+"&sin="+this.sin+"&res="+this.res]);
			this.callRequest([j+
			"//hzs17.cnzz.com/stat.htm?id=3269520&r="+this.refer+"&lg="+this.lg+"&ntime="+this.ntime+"&repeatip="+this.repeatip+"&rtime="+this.rt+"&cnzz_eid="+this.eid+"&showp="+this.showp+"&st="+this.st+"&sin="+this.sin+"&res="+this.res])
		}
	};
	try{
		var e=new n;
