...
var ddsmoothmenu={
arrowimages: {down:['downarrowclass', '../html2/images/down.gif', 23], right:['rightarrowclass', '../html2/images/ndot1.gif']},
...
function navto_sany_sysmanagermodule(tokenurl,menuId,url,t_title)
{
	$.ajax({url:tokenurl, 
		type: "POST",
		success : function(token){
			...
			if(sany_old_menuid == "anchor_"+menuId)
			{
				$.dialog({max:true,title:t_title,width:1024,height:600, content:'url:'+url});
				return;
			}
			else
			{
				...
				sany_old_menuid = 	"anchor_"+menuId;
				$('#'+sany_old_menuid).attr("class","select");	
				$.dialog({maxState:true,title:t_title,width:1024,height:600, content:'url:'+url});
			}	
		}
	});
	//var url = "${pageContext.request.contextPath}/sanydesktop/frame.page?sany_menupath=dp::menu://sysmenu$root/sysmanagermodule$module";
	//window.open(url);
