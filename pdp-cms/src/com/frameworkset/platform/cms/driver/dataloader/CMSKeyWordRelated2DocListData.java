...
public class CMSKeyWordRelated2DocListData extends CMSBaseListData implements java.io.Serializable {
	/**
	* 分页显示时获取每页的数据项
	* sortKey:排序字段
		...
		try {
			if(context != null){
				...
				ListInfo lstifo = (CMSUtil.getCMSDriverConfiguration()
							  .getCMSService()
							  .getDocumentManager()
							  .getKeyWord2RelationOfDocument(contentctx.getDocument().getSubtitle(),
				...
				ListInfo lstifo = (CMSUtil.getCMSDriverConfiguration()
							  .getCMSService()
							  .getDocumentManager()
							  .getKeyWord2RelationOfDocument(null,
		...
		try {
			if(context != null){
				...
				lstifo.setDatas(CMSUtil.getCMSDriverConfiguration()
							  .getCMSService()
							  .getDocumentManager()
							  .getKeyWord2RelationOfDocument(null,Integer.parseInt(documentid),channel,siteinfo.getSiteId() + "",count));
				return lstifo;
			}
		} catch (DocumentManagerException e) {
			e.printStackTrace();
