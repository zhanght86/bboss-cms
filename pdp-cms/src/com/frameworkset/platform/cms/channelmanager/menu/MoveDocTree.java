...
public class MoveDocTree extends COMTree implements java.io.Serializable {
	...
	public boolean hasSon(ITreeNode father) {
		...
		try {  
			...
			if (type.equals("site") || type.equals("site_true")) { 
				...
				//return channelManager.hasSubChannel(tmp[0]);
		...
		try {
			...
			if (father.isRoot() || father.getType().equals("site") || father.getType().equals("site_true")) 
			{
				...
				if(!parentSiteid.equals("0"))
				{
					...
					//List channels = siteManager.getDirectChannelsOfSite(parentSiteid);
...
//									|| accesscontroler.checkPermission(siteid,AccessControl.CHANNEL_ADDDOC_PERMISSION,AccessControl.SITEDOC_RESOURCE)
							if (accessControl.checkPermission(String.valueOf(channel.getChannelId()),
									AccessControl.CHANNEL_ADDDOC_PERMISSION, AccessControl.CHANNELDOC_RESOURCE) ||
									accessControl.checkPermission(String.valueOf(channel.getSiteId()),
											AccessControl.CHANNEL_ADDDOC_PERMISSION, AccessControl.SITEDOC_RESOURCE)) {
								addNode(father, channel.getChannelId() + ":" + parentSiteid, treeName, nodeType,
										false, curLevel, (String) null, 
					...
					while (iterator.hasNext()) {
		            	...
		            	if (accessControl.checkPermission(String.valueOf(channel.getChannelId()),
						    ...
						    addNode(father, String.valueOf(channel.getChannelId()) + ":" + siteid, treeName, nodeType,
									false, curLevel, (String) null, 
									siteid + ":" +  String.valueOf(channel.getChannelId()),
									siteid + ":" +  String.valueOf(channel.getChannelId()), map);
		            	}
		            	else
		            	{
		            		addNode(father, String.valueOf(channel.getChannelId()) + ":" + siteid, treeName, nodeType,
