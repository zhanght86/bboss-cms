...
public class TestDemo {
	...
	public static void deploytest() {
		ActivitiServiceImpl service = new ActivitiServiceImpl(
				"activiti.cfg.xml");
		service.deployProcDefByPath("测试1",
				"/com/sany/activiti/demo/diagrams/mms.get.zip");
		service.deployProcDefByPath("测试2",
				"/com/sany/activiti/demo/diagrams/diagrams1.zip");
	}
	...
	public static void testinitor() {
		...
		params.put("usertask2_user",
				Arrays.asList("user2,user22,user23".split(",")));
		...
		{
			// ProcessInstance instance = activitiService.startProcDef("demo1",
			// params);
			// ProcessInstance instance =
			// activitiService.startProcDef(UUID.randomUUID().toString(),"demo1",
			// params);
			ProcessInstance instance = activitiService.startProcDef(UUID
					.randomUUID().toString(), "demo1", params, "user1");
		...
		for (String user : users) {
			List<Task> taskList1 = activitiService.getTaskService()
					.createTaskQuery().taskCandidateUser(user).list();
			for (Task task : taskList1) {

				System.out.println("=========获取流程各节点审批人信息");
				Map variables = activitiService.getRuntimeService()
				...
				// activitiService.commitProcess(task.getId(), user, new
				// HashMap(), "usertask3");

				System.out.println("=========查询历史记录");
				// List his =
				// activitiService.findHistoricUserTask(task.getProcessInstanceId());
	...
	public static void checkOne() {
		...
		try {
			...
			List<Deployment> deployments = activitiService
					.getRepositoryService().createDeploymentQuery()
					.deploymentName("demo1").orderByDeploymenTime().desc()
					.list();
			...
			ProcessDef def = activitiService
			...
			params.put("usertask2_user",
					Arrays.asList("user2,user22,user23".split(",")));
			params.put("usertask3_user",
					Arrays.asList("user3,user32".split(",")));
			params.put("usertask4_user",
					Arrays.asList("user4,user42".split(",")));
			params.put("usertask5_user", Arrays.asList("user5".split(",")));
			params.put("businessType", "test");
			...
			{
				// ProcessInstance instance =
				// activitiService.startProcDef("demo1", params);
				// ProcessInstance instance =
				// activitiService.startProcDef(UUID.randomUUID().toString(),"demo1",
				// params);
				ProcessInstance instance = activitiService.startProcDef("test",
						"demo1", params);
				System.out.println("processDefinitionId="
						+ instance.getProcessDefinitionId());
				System.out.println("businessKey=" + instance.getBusinessKey());
			...
			for (String user : users) {
				...
				for (Task task : taskList1) {
					...
					activitiService.completeTask(task.getId(), user,
							common_params, null);
			...
			for (String user : users1) {
				...
				for (Task task : taskList1) {
					...
					// activitiService.findBackAvtivity(task.getId());
					// System.out.println("节点列表："+activityList);

					System.out.println("==============================驳回任意节点");
					String toTaskKey = "usertask1";// 驳回节点id
					activitiService.completeTask(task.getId(), user,
							common_params, toTaskKey);
					// activitiService.commitProcess(task.getId(), user,
					// common_params, toTaskKey);
					// activitiService.rejectProcess(task.getProcessInstanceId(),
					// toTaskKey,task.getTaskDefinitionKey(),user, new
					// HashMap());
					//
			...
			users = new String[] { "user1", "user2", "user22", "user23",
					"user5" };
			for (String user : users) {
				List<Task> taskList1 = activitiService.getTaskService()
						.createTaskQuery().taskCandidateUser(user).list();
			...
			List<ProcessInstance> instanceList = activitiService
					...
					.processDefinitionKey("demo1").active().list();
			System.out.println("==============================获取实例的待办");
			for (ProcessInstance instance : instanceList) {
				HistoricProcessInstance hisInstance = activitiService
						.getHistoryService()
				...
				System.out.println("businessKey=" + instance.getBusinessKey());
				System.out.println("instanceEnded="
						+ (hisInstance.getEndTime() != null));
	...
	public static void huiqianbohui() {
		...
		try {
			...
			// if (instanceList.isEmpty())
			{
				...
				// params);
				ProcessInstance instance = activitiService.startProcDef(UUID
						.randomUUID().toString(), "demo1", params);
				System.out.println("processDefinitionId="
						+ instance.getProcessDefinitionId());
				System.out.println("businessKey=" + instance.getBusinessKey());
				System.out.println("instanceId="
	...
	public static void huiqianparraleltosequential() {
		...
		try {
			...
			params.put("businessType", "test");
			/**
			 * 会签任务usertask2流程模型中被定位并行模式，
			 * 这里在流程实例启动时通过流程变量将会签任务usertask2的并行模式改为串行模式
		...
		try {
			...
			for (String user : users) {
				...
				for (Task task : taskList1) {
					//
					/**
					 * 会签任务usertask2流程模型中已经被切换为串行模式，
					 * 这里在完成usertask1任务时通过流程变量将会签任务usertask2的串行模式再改为并行模式
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	public static void deployment(String deploymentkey, String path,
			String imagePath) {
		TransactionManager tm = new TransactionManager();
		...
		try {
			...
			Deployment deployment = activitiService.deployProcDefByPath(
					deploymentkey, path, imagePath,
			...
			List<ActivityImpl> activities = activitiService
					.getActivitImplListByProcessKey(psKey);
			for (ActivityImpl actImpl : activities) {
				...
				if (actType.equals("userTask") || actType.equals("subProcess")) {
					...
					if (actType.equals("subProcess")) {
						List<ActivityImpl> _activities = actImpl
								.getActivities();
