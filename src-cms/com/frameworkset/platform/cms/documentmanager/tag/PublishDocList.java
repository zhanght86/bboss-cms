package com.frameworkset.platform.cms.documentmanager.tag;

import com.frameworkset.platform.cms.documentmanager.DocumentManagerImpl;
import com.frameworkset.common.poolman.DBUtil;
import com.frameworkset.common.tag.pager.DataInfoImpl;
import com.frameworkset.util.ListInfo;

public class PublishDocList extends DataInfoImpl implements java.io.Serializable {

	protected ListInfo getDataList(String sortKey, boolean desc, long offset,
            int maxPagesize) {		
		String curUserid = (String)request.getAttribute("curUserid");
		String docTitle = request.getParameter("docTitle");
		String channelIds = request.getParameter("channelIds");
		if(channelIds!=null && channelIds.endsWith(",")){
			channelIds = channelIds.substring(0,channelIds.length()-1);
		}
		String submitter = request.getParameter("submitter");
		String subTime = request.getParameter("subTime");
		String docFlow = request.getParameter("docFlow");
		String flag = request.getParameter("flag");//查询所有标志
		String detailTemplateFlag = request.getParameter("detailTemplateFlag");

		ListInfo listInfo=null;
		DocumentManagerImpl docManager = new DocumentManagerImpl();
		try{
			String sql = "select a.task_id as task_id, c.channel_id as channel_id, a.document_id " +
					"as document_id, a.submit_time as submit_time,f.name as docSiteName,c.doctype as doctype," +
					" c.subtitle as subtitle, d.user_name as user_name, e.name as channel_name,c.flow_id as flow_id,e.site_id as site_id " +
					"from td_cms_doc_task a, td_cms_doc_task_detail b, " +
					"td_cms_document c, td_sm_user d ,td_cms_channel e,td_cms_site f " +
					"where c.isdeleted!=1 and a.pre_status=11 and c.status=11 and a.task_id=b.task_id and c.document_id=a.document_id " +
					"and e.channel_id=c.channel_id and d.user_id=a.submit_id and f.site_id=e.site_id and (f.status=0 or f.status=1) and " +
					"b.performer = " + curUserid + " and b.valid = 1 and complete_time is null " ;
			if(flag !=null && !flag.equals("all") && detailTemplateFlag != null && detailTemplateFlag.equals("2"))
				sql = sql + " and (e.detail_tpl_id is null or e.detail_tpl_id = '') ";
			else if(flag !=null && !flag.equals("all") && detailTemplateFlag != null && detailTemplateFlag.equals("1"))
				sql = "select a.task_id as task_id, c.channel_id as channel_id, a.document_id " +
					"as document_id, a.submit_time as submit_time,f.name as docSiteName,c.doctype as doctype," +
					" c.subtitle as subtitle, d.user_name as user_name, e.name as channel_name,c.flow_id as flow_id,e.site_id as site_id " +
					"from td_cms_doc_task a, td_cms_doc_task_detail b, " +
					"td_cms_document c, td_sm_user d ,td_cms_channel e,td_cms_site f,td_cms_template g " +
					"where c.isdeleted!=1 and a.pre_status=11 and c.status=11 and a.task_id=b.task_id and c.document_id=a.document_id " +
					"and e.channel_id=c.channel_id and d.user_id=a.submit_id and f.site_id=e.site_id and (f.status=0 or f.status=1) and e.detail_tpl_id=g.template_id and g.type = '2' and " +
					"b.performer = " + curUserid + " and b.valid = 1 and complete_time is null " ;
			if((docTitle == null && submitter == null && channelIds == null && subTime == null && docFlow == null && detailTemplateFlag == null && curUserid != null)   
					|| (docTitle.length()==0 && submitter.length()==0 && channelIds.length()==0 && subTime.length()==0 && docFlow.length()==0 && detailTemplateFlag.length()==0 && curUserid != null) 
					|| (flag !=null && flag.equals("all")) 
					|| (flag == null)){
				sql += " order by a.submit_time desc,e.channel_id,a.document_id desc";
				listInfo = docManager.getPubishDocList(sql,(int)offset,maxPagesize);
			}
			else{
				if(docTitle != null && docTitle.length()!=0)
					sql += " and c.subtitle like '%" + docTitle + "%'";
				if(channelIds != null && channelIds.length()!=0)
					sql += " and c.channel_id in(" + channelIds + ")";
				if(submitter != null && !submitter.equals(""))
					sql += " and d.user_name like '%" + submitter + "%'";
				if(docFlow != null && docFlow.length()!=0)
					sql += " and c.flow_id = " + docFlow;
				if(subTime != null && !subTime.equals(""))
					sql += " and a.submit_time <" + DBUtil.getDBDate(subTime) + "+1";
				sql += " order by a.submit_time desc,e.channel_id,a.document_id desc";
				listInfo = docManager.getPubishDocList(sql,(int)offset,maxPagesize);
			}	
		}catch(Exception ex){
			ex.printStackTrace();
			}
		return listInfo;
	}

	protected ListInfo getDataList(String arg0, boolean arg1) {
		// TODO Auto-generated method stub
		return null;
	}

}