...
public class DocCommentManagerImpl implements DocCommentManager {
	...
	public void updateCommentStatus(String docCommentId, int newStatus) throws DocCommentManagerException {
		...
		String sql = "update td_cms_doc_comment set status = " + newStatus + " where comment_id = " + docCommentId;
		try {
			db.executeUpdate(sql);
		} catch (SQLException e) {
			e.printStackTrace();
			throw new DocCommentManagerException("更新指定评论的状态失败！" + e.getMessage());
		}
	}
	...
	public void addOneComment(DocComment docComment) throws DocCommentManagerException {
		...
		try {
			...
			int id = (Integer) dbUitl.executeSelectForObject(executor.getSql("getCommentNextId"), Integer.class);
	...
	public void delCommentsByDocId(int docId) throws DocCommentManagerException {
		...
		String sql1 = "delete from td_cms_doccom_impeachinfo b " + "where b.comment_id in "
				+ "(select a.comment_id from td_cms_doc_comment a " + "where a.doc_id = " + docId + ")";
		try {
			db.executeDelete(sql1);
			db.executeDelete(sql);
		} catch (SQLException e) {
			e.printStackTrace();
			throw new DocCommentManagerException("删除指定文档的评论失败！" + e.getMessage());
		}
	}
	...
	public void delOneCommentByComId(int docCommentId) throws DocCommentManagerException {
		// TODO Auto-generated method stub
		DBUtil db = new DBUtil();
		String sql = "delete from td_cms_doc_comment where comment_id = " + docCommentId;
		try {
			db.executeDelete(sql);
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public int getCommentCount(int docId) throws DocCommentManagerException {
		DBUtil db = new DBUtil();
		String sql = "select comment_id from td_cms_doc_comment where doc_id = " + docId;
		try {
			db.executeSelect(sql);
			return db.size();
		} catch (SQLException e) {
			e.printStackTrace();
			throw new DocCommentManagerException("统计指定文档的评论总数失败！" + e.getMessage());
		}
	}

	public int getCommentPublishedCount(int docId) throws DocCommentManagerException {
		DBUtil db = new DBUtil();
		String sql = "select comment_id from td_cms_doc_comment where status = 1 and doc_id = " + docId;
		try {
			db.executeSelect(sql);
			return db.size();
		} catch (SQLException e) {
	...
	public ListInfo getCommnetList(int docId, int offset, int maxItem) throws SQLException {
		...
		return executor.queryListInfoBean(DocComment.class, "getCommnetList", offset, maxItem, paramMap);
	}
	...
	public int[] getResponseInfo(int docCommentId) throws DocCommentManagerException {
		...
		DBUtil db = new DBUtil();
		try {
			String sql = "select * from td_cms_doc_comment where src_comment_id = " + docCommentId
					+ " and doc_comment like '顶'";
			db.executeSelect(sql);
			temp[0] = db.size(); // "顶"人数
			sql = "select * from td_cms_doc_comment where src_comment_id = " + docCommentId
					+ " and doc_comment like '不好说'";
			db.executeSelect(sql);
			temp[1] = db.size(); // "不好说"人数
			sql = "select * from td_cms_doc_comment where src_comment_id = " + docCommentId
					+ " and doc_comment like '反对'";
			db.executeSelect(sql);
			temp[2] = db.size(); // "反对"人数
			sql = "select * from td_cms_doc_comment where src_comment_id = " + docCommentId
					+ " and doc_comment like '不知所云'";
			db.executeSelect(sql);
	...
	public void switchDocCommentFunction(String id, String commentSwitch, String docorchnl)
		...
		if (docorchnl.equals("doc")) {
			if (commentSwitch.equals("open")) { // 开通文档评论功能
				sql = "update td_cms_document set commentswitch = 0 where document_id = " + id;
			} else if (commentSwitch.equals("close")) { // 关闭文档评论功能
				sql = "update td_cms_document set commentswitch = 1 where document_id = " + id;
			}
		} else if (docorchnl.equals("chnl")) {
			if (commentSwitch.equals("open")) { // 关闭频道评论功能
				sql = "update td_cms_channel set commentswitch = 0 where channel_id in "
				...
				sql = "update td_cms_channel set commentswitch = 1 where channel_id in "
						+ "(select channel_id from td_cms_channel connect by parent_id = prior channel_id start with channel_id='"
						+ id + "')";
		...
		try {
			...
			e.printStackTrace();
			throw new DocCommentManagerException("开通（关闭）评论功能时，数据库操作失败！" + e.getMessage());
		}
	}
	...
	public int getDocCommentSwitch(String id, String docorchnl) throws DocCommentManagerException {
		...
		int commentSwitch = 0;
		if (docorchnl.equals("doc")) {
			sql = "select commentswitch from td_cms_document where document_id = " + id;
		} else if (docorchnl.equals("chnl")) {
			sql = "select commentswitch from td_cms_channel where channel_id = " + id;
		}
		try {
			db.executeSelect(sql);
			if (db.size() > 0)
			...
			e.printStackTrace();
			throw new DocCommentManagerException("获取评论功能信息时，数据库操作失败！" + e.getMessage());
		}
	}
	...
	public void deleteImpeachInfo(String impeacheId) throws DocCommentManagerException {
		DBUtil db = new DBUtil();
		String sql = "delete from td_cms_doccom_impeachinfo where id=" + impeacheId;
		try {
			db.executeDelete(sql);
		} catch (SQLException e) {
			e.printStackTrace();
	...
	public void replyImpeach(String impeacheId, String toEmail, String toName, String subject, String msg)
			throws DocCommentManagerException {
		try {
			...
			// 更新为已回复
			DBUtil db = new DBUtil();
			String sql = "update td_cms_doccom_impeachinfo set replyed=1 where id=" + impeacheId;
			db.executeUpdate(sql);
		} catch (Exception e) {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.frameworkset.platform.cms.docCommentManager.DocCommentManager#getChannelCommentAduitSwitch(int)
	 */
	@Override
	...
	/* (non-Javadoc)
	 * @see com.frameworkset.platform.cms.docCommentManager.DocCommentManager#switchDocCommentAudit(int, int)
	 */
	@Override
