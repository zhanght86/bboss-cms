...
/**
 * 频道列表数据加载器
 * <p>Title: CMSChannelListData.java</p>
 *
 * <p>Description: </p>
 *
 ...
 *
 * <p>Company: 三一集团</p>
 * @Date 2007-5-31 18:22:31
 * @author kai.hu
 * @version 1.0
 */
public class CMSChannelListData extends CMSBaseListData implements java.io.Serializable {
				...
				if(context != null)
				{
					if(this.site == null)
					{
						ListInfo lstifo = (CMSUtil.getCMSDriverConfiguration()
									  .getCMSService()
									  .getChannelManager()
									  .getDirectSubChannels(super.context.getSiteID(),channel,offset,maxpageitems));
						return lstifo;
					}
					else
					{
						...
						ListInfo lstifo = (CMSUtil.getCMSDriverConfiguration()
									  .getCMSService()
									  .getChannelManager()
									  .getDirectSubChannels(siteinfo.getSiteId()+"",channel,offset,maxpageitems));
						return lstifo;
					}
				}
				else
		...
		try {
			if(this.channel != null && !this.channel.equals(""))
			{
				...
				if(context != null)
				{
					if(this.site == null)
					{
						List published = CMSUtil.getCMSDriverConfiguration().getCMSService().getChannelManager().getDirectSubChannels(context.getSiteID(),channel,count);
					...
					else
					{
						...
						List published = CMSUtil.getCMSDriverConfiguration()
												.getCMSService()
												.getChannelManager()
												.getDirectSubChannels(siteinfo.getSiteId() + "",channel,count);
						ListInfo listInfo = new ListInfo();
						listInfo.setDatas(published);
						return listInfo;
					}
