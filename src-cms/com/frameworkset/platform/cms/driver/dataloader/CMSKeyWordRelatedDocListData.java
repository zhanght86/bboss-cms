...
public class CMSKeyWordRelatedDocListData extends CMSBaseListData implements java.io.Serializable {
	/**
	* 分页显示时获取每页的数据项
	* sortKey:排序字段
		...
		try {
			if(context != null){
				...
				ListInfo lstifo = (CMSUtil.getCMSDriverConfiguration()
							  .getCMSService()
							  .getDocumentManager()
							  .getKeyWordRelationOfDocument(contentctx.getDocument().getKeywords(),
				...
				ListInfo lstifo = (CMSUtil.getCMSDriverConfiguration()
							  .getCMSService()
							  .getDocumentManager()
							  .getKeyWordRelationOfDocument(null,
		...
		try {
			if(context != null){
				...
				lstifo.setDatas(CMSUtil.getCMSDriverConfiguration()
							  .getCMSService()
							  .getDocumentManager()
							  .getKeyWordRelationOfDocument(null,Integer.parseInt(documentid),siteinfo.getSiteId() + "",count));
				return lstifo;
			}
		} catch (DocumentManagerException e) {
			e.printStackTrace();
