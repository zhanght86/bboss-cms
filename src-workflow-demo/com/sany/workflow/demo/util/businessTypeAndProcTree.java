 ...
 */
public class businessTypeAndProcTree extends COMTree {

	private static final long serialVersionUID = 1L;

	private static Logger logger = Logger
			.getLogger(businessTypeAndProcTree.class);
	...
	public boolean setSon(ITreeNode father, int curLevel) {
		...
		try {
			...
			if (father.getId().equals("0")) {
				businessTypeList = businessTypeService.getSonNodes(father
						.getId());
			} else {
				businessTypeList = businessTypeService.getSonNodes(father
						.getId());
			...
			if (businessTypeList != null) {
				...
				for (BusinessType temp : businessTypeList) {
					...
					map.put("nodeLink", "javascript:query('" + businessTypeId
							+ "','"
							+ WorkFlowConstant.BUSINESS_TYPE_BUSINESSTYPE
							+ "','" + businessTypeName + "');");
					addNode(father, businessTypeId, businessTypeName,
							"businessType", true, curLevel, (String) null,
							(String) businessTypeId, (String) businessTypeId,
							map);
				}
			}
