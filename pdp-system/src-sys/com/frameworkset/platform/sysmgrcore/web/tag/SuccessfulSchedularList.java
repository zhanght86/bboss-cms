...
public class SuccessfulSchedularList extends DataInfoImpl implements Serializable{
	protected ListInfo getDataList(String sortKey, boolean desc, long offset,
		...
		try {
			String sql = "select * from TD_SD_SCHEDULAR where status = 0 and endtime >= "
					+ SQLManager.getInstance().getDBAdapter().getDateString(dt)
					+ "  and executor_id = " + j + " order by schedular_id desc";
