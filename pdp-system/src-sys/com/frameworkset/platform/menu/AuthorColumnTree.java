 ...
 * <p>Title: </p>
 *
 * <p>Description: 菜单管理授权树,应用于系统管理中菜单管理模块的管理树,用户管理菜单授权树</p>
 *
 ...
 * @version 1.0
 */
public class AuthorColumnTree extends COMTree implements Serializable{
    ...
    public boolean hasSon(ITreeNode father) { 
        ...
        else 
        {
        	...
        	else if(father.getType().equals("reportmenu") || father.getType().equals("reportmenutrue"))//如果是cs的菜单,调用cs的接口进行判断father是否有下级菜单
        	{
        ...
        else if(father.getType().equals("subsystem"))
        {
        	...
        	else if(father.getId().equals("reportmenu"))
        	{
            	...
            	for(int i = 0; i < topLevels.size(); i ++)
            	{
    				...
    				{
        				addNode(father, treeid, treeName, type, true, curLevel, "",
        	                    (String)null, (String)null, (String)null,params);
    				}
            		else
            		{
            			addNode(father, treeid, treeName, type, false, curLevel, "",
        	                    (String)null, (String)null, (String)null,params);
            		}
            	}
            	return true;
...
//        				params.put("resId2",menu.getId());        				
        ...
        else if(father.getType().equals("reportmenu") || father.getType().equals("reportmenutrue"))
        {
        ...
        for(int i = 0; i < submodules.size(); i ++)        {
            ...
            params.put("columnID",treeid);
            params.put("resId",submodule.getId().trim());
           	params.put("resName",treeName);
			if(AccessControl.hasGrantedRole(roleId,roleTypeId,submodule.getId().trim(),resTypeId)){
