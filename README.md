...
(
  ...
  TASK_TYPE        NUMBER(1)                    DEFAULT 0,
   DEALERNAME NVARCHAR2(255),
  SENDERWORKNO     VARCHAR2(50 BYTE)
);
...
COMMENT ON COLUMN TD_WF_RUN_TASK.DEALER IS '任务处理人账号';

COMMENT ON COLUMN TD_WF_RUN_TASK.TASK_TYPE IS '任务类型，0-普通任务类型 1-抄送通知任务类型';

COMMENT ON COLUMN TD_WF_RUN_TASK.SENDERWORKNO IS '提交人工号';
...
oracle升级脚本：
ALTER TABLE TD_WF_RUN_TASK
ADD (DEALER NVARCHAR2(255))

COMMENT ON COLUMN 
TD_WF_RUN_TASK.DEALER IS 
'任务处理人账号';

ALTER TABLE TD_WF_RUN_TASK
ADD (Task_type NUMBER(1) DEFAULT 0)

COMMENT ON COLUMN 
TD_WF_RUN_TASK.Task_type IS 
'任务类型，0-普通任务类型 1-抄送通知任务类型';

ALTER TABLE TD_WF_RUN_TASK
ADD (senderworkno VARCHAR2(50 BYTE));

COMMENT ON COLUMN 
TD_WF_RUN_TASK.senderworkno IS 
'提交人工号';
ALTER TABLE TD_WF_RUN_TASK ADD (DEALERNAME NVARCHAR2(255));
ALTER TABLE TD_WF_RUN_TASK DROP CONSTRAINT TD_WF_RUN_TASK_PK;

ALTER TABLE TD_WF_RUN_TASK ADD PRIMARY KEY (TASK_ID, DEALER);